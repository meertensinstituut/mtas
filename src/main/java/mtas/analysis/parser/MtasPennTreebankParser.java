...
public class MtasPennTreebankParser extends MtasParser {
  ...
  public MtasPennTreebankParser(Configuration config) {
    super(config);
    try {
      initParser();
  ...
  protected void initParser() throws MtasConfigException {
    ...
    if (config != null) {
      for (int i = 0; i < config.numChildren(); i++) {
        ...
        if (current.getName().equals(PENNTREEBANK_IGNORE)) {
          for (int j = 0; j < current.numChildren(); j++) {
            if (current.child(j).getName().equals(PENNTREEBANK_NODE)) {
              String nameVariable = current.child(j).getAttr(PENNTREEBANK_NODE_NAME);
