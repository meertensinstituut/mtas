...
RUN solrurl=$(curl -s 'http://www.apache.org/dyn/closer.lua/lucene/solr/${currentLuceneVersion}/solr-${currentLuceneVersion}.tgz' |   grep -o '<strong>[^<]*</strong>' |   sed 's/<[^>]*>//g' |   head -1) && \
    if echo "$solrurl" | grep -q '^.*[^ ].*$'; then \
      curl -f -o /root/solr-${currentLuceneVersion}.tgz -O $solrurl || true; \
    fi && \
    if [ ! -f /root/solr-${currentLuceneVersion}.tgz ]; then \
      echo "Solr ${currentLuceneVersion} not found in mirror, falling back to apache archive"; \ 
      solrurl="http://archive.apache.org/dist/lucene/solr/${currentLuceneVersion}/solr-${currentLuceneVersion}.tgz"; \
      curl -f -o /root/solr-${currentLuceneVersion}.tgz -O $solrurl; \ 
    fi && \    
    tar xzf solr-${currentLuceneVersion}.tgz solr-${currentLuceneVersion}/bin/install_solr_service.sh --strip-components=2 && \
    bash ./install_solr_service.sh solr-${currentLuceneVersion}.tgz && rm install_solr_service.sh && rm -rf solr-${currentLuceneVersion}.tgz
